...
@section('styles')
    <link rel="stylesheet" href="{{ URL::asset('css/manager/receipt-payment.css') }}">
...
@section('content')
    <h1 class="h3 mb-4 text-gray-800 main-title"> سند الصرف </h1>
    <div class="container">

	<form action="{{ route('receipt-payment-edit', ['id' => $receipt->id] ) }}" class="form-admin" id="receipt_payment_submit" method="post">
     {{ csrf_field() }}
	 <input type="hidden" name="allcheques" id="allcheques">
		...
		{{$receipt->invoice_number}} سند صرف رقم</div>
		...
		<div class = "row" style="direction: rtl;">
					...
					<div class="form-group col-6">
						<select dir="rtl" class="form-control js-example-basic-single" id="customer-name" name="customer" data-validation="required" required> 
							...
							@foreach( $customers as $customer)
								@if($receipt->client_type.'.'.$receipt->customer_id == 'C.'.$customer->id) 
							...
							@foreach( $suppliers as $supplier)
								@if($receipt->client_type.'.'.$receipt->supplier_id == 'S.'.$supplier->id) 
									<option dir="rtl" class="text-right" value="S.{{ $supplier->id }}" selected>{{ $supplier->name }} </option>
								@else
									<option dir="rtl" class="text-right" value="S.{{ $supplier->id }}">{{ $supplier->name }}  </option>
								@endif
							@endforeach 

							@foreach( $employees as $employee)
								@if($receipt->client_type.'.'.$receipt->employee_id == 'E.'.$employee->id) 
									<option dir="rtl" class="text-right" value="E.{{ $employee->id }}" selected>{{ $employee->name }}  </option>
								@else
									<option dir="rtl" class="text-right" value="E.{{ $employee->id }}">{{ $employee->name }}  </option>
								@endif
							@endforeach
						</select>
					</div>
				...
				<table class="table table-bordered table-hover text-center">
					  ...
					  <td id="cashcurrency_basecurrency" class="readonlycell">{{ $organization_currency->name }}</td>
						...
						@if($receipt->receiptsCash()->contains('currency_id', $organization_currency->currency_id))
							@foreach($receipt->receiptsCash() as $receiptCash)
								...
								<input type="text" id="cash_basecurrency" name="cash_basecurrency" 
								value="{{$receiptCash->amount}}"
								class="inputwithoutborder text-center">								
								 </td>
				  ...
				  @if($receipt->receiptsCash()->contains('currency_id', $currency->id))
					  ...
					  <td id="cashvalue_{{ $currency->id }}">
						<input type="text" id="cash_{{ $currency->id }}" value="{{$receiptCash->amount}}" name="cash_{{ $currency->id }}" class="inputwithoutborder text-center">
					  </td>
					  <td id="cashexchange_{{ $currency->id }}">
						<input type="text" id="cashexchangeinput_{{ $currency->id }}" value="{{ $receiptCash->exhange_rate }}" name="cashexchangeinput_{{ $currency->id }}" class="inputwithoutborder text-center">							
					  </td>
			...
			<div class="col-12 col-md-5">
				...
				<div class="col-6">
					<div class="mx-auto form-group">
							<input type="text" class="form-control" id="datepicker_main" name="datepicker_main" value="{{$receipt->receipt_date}}" required readonly style="background:white;">
						</div>
				</div>
				</div>
			...
			<div class="table-responsive">
				...
				<div class="col-12 text-center" style="border: 1px solid #aaa;height:40px;">
					<input type="text" id="notes" name="notes" value="{{$receipt->notes}}" class="inputwithoutborder text-center col-12" style="height: 40px; max-width: 100%; padding: 0px; margin: 0px;">
				</div>				
		...
		<div class="form-row exchange-rate-main" id="btn-exchange-rate">
			<div class="form-group col-md-3 col-sm-3 col-xs-12 right">
				<button type="button" id="save" class="btn btn-success btn-block" onclick="submitFunction()">تعديل</button>
			</div>
			<div class="form-group col-md-3 col-sm-3 col-xs-12 right">
...
@section('scripts')
	...
	@if(Session::has('addReceiptPayment'))
<script type="text/javascript">
window.setTimeout(function setTimeout() {
                    $.bootstrapPurr('{{Session::get("addReceiptPayment")}}', {
                    type: 'success',
                    delayPause: true,
                    width: 'auto',                    
                    allowDismiss: true,
...
@endif
		...
		@if(Session::has('errorReceiptPayment'))
<script type="text/javascript">
window.setTimeout(function setTimeout() {
                    $.bootstrapPurr('{{Session::get("errorReceiptPayment")}}', {
                    type: 'danger',
                    delayPause: true,
                    width: 'auto',                    
                    allowDismiss: true,
...
function exchange(id) {
	...
	function submitFunction() {
			...
			document.getElementById("receipt_payment_submit").submit();
