...
@section('content')
	...
	<form action="{{ route('receipt-edit', ['id' => $receipt->id] ) }}" class="form-admin" id="receipt_submit" method="post">
     {{ csrf_field() }}
					  ...
					  <td id="cashcurrency_basecurrency" class="readonlycell">{{ $organization_currency->name }}</td>
						...
						@if($receipt->receiptsCash()->contains('currency_id', $organization_currency->currency_id))
							@foreach($receipt->receiptsCash() as $receiptCash)
								...
								<input type="text" id="cash_basecurrency" name="cash_basecurrency" value="{{$receiptCash->amount}}" class="inputwithoutborder text-center">								
								 </td>
