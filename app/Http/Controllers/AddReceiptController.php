...
class AddReceiptController extends Controller {
    public function show() {
        ...
        return view('manager.pages.receipt-add')->with(['organization_currency' => $organization_currency,
    ...
    public function add(Request $request) {
		...
		DB::transaction(function() use ($request, $organization_currency, $currencies, $array) {
				...
				Session::flash('addReceipt','تم إضافة سند القبض بنجاح');

                
            });	

		} else {
			Session::flash('errorReceipt','يرجى تعبئة الحقول المطلوبة');
		}
